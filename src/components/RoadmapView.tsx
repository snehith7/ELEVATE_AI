...
export const RoadmapView: React.FC<RoadmapViewProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'quest-map' | 'codex'>('quest-map');
  ...
  return (
    <div className="relative max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-6 sm:space-y-8">
      ...
      {/* Hero Banner with Coral Theme & Live Rank Ring */}
      <div className="relative z-10 rounded-3xl overflow-hidden bg-gradient-to-r from-[#170e10] via-[#0d0d14] to-[#07070a] border border-[#2b1c22] p-4 sm:p-8 shadow-2xl backdrop-blur-xl">
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-[#FF5A43]/15 border border-[#FF5A43]/30 text-[#FF8570] text-xs font-semibold">
              ...
              <span>Adaptive Quest Academy • Level Up Your Coding</span>
            </div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-white tracking-tight">
              Master Coding Fundamentals to Advanced Systems
            </h1>
            <p className="text-slate-300 text-sm leading-relaxed">
              Explore interconnected quest realms, tackle live algorithmic challenges, and unlock badges. Our Gemini AI evaluates submissions with instant diagnostics and generates targeted practice to conquer your weak spots.
            </p>

            {/* View Mode Switcher */}
            <div className="flex items-center space-x-2 pt-2">
              <button
                id="roadmap-toggle-quest-map"
                onClick={() => {
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  viewMode === 'quest-map'
                    ? 'bg-[#FF5A43] text-white shadow-lg shadow-[#FF5A43]/30 ring-2 ring-[#FF5A43]/50'
                    : 'bg-[#141420] text-slate-300 hover:text-white hover:bg-[#1b1b28] border border-[#242436]'
                }`}
              >
              ...
              >
                ...
                <span>Curriculum Codex ({problems.length})</span>
              </button>
            </div>
          </div>

          {/* Right Side: Mini Live Rank Progress Ring in Hero */}
      ...
      {/* MAIN VIEW MODE: Quest Map vs Curriculum Codex */}
      {viewMode === 'quest-map' ? (
        ...
        <div className="relative z-10 space-y-6">
          {/* 3-Tier Roadmap Track Overview Cards with 3D Tilt */}
            ...
            <TiltCard3D
              ...
              className={`rounded-2xl border transition-all cursor-pointer ${
                selectedDifficulty === 'basic'
                  ? 'bg-[#121c15] border-emerald-500 ring-2 ring-emerald-500/40'
                  : 'bg-[#0e0e15] border-[#1c1c28] hover:border-[#2a2a3d]'
              }`}
            >
              <div className="p-5">
            ...
            <TiltCard3D
              ...
              className={`rounded-2xl border transition-all cursor-pointer ${
                selectedDifficulty === 'intermediate'
                  ? 'bg-[#1e1710] border-amber-500 ring-2 ring-amber-500/40'
                  : 'bg-[#0e0e15] border-[#1c1c28] hover:border-[#2a2a3d]'
              }`}
            >
              <div className="p-5">
            ...
            <TiltCard3D
              ...
              className={`rounded-2xl border transition-all cursor-pointer ${
                selectedDifficulty === 'advanced'
                  ? 'bg-[#201014] border-[#FF5A43] ring-2 ring-[#FF5A43]/40'
                  : 'bg-[#0e0e15] border-[#1c1c28] hover:border-[#2a2a3d]'
              }`}
            >
              <div className="p-5">
          ...
          {/* Filter & Search Bar */}
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 p-4 rounded-2xl bg-[#0e0e15]/90 border border-[#1c1c28] backdrop-blur-xl">
          ...
          {/* Problems List Grid with 3D Depth */}
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-slate-400 px-1">
              <span>Showing {filteredProblems.length} practice problems in Codex</span>
              ...
              {filteredProblems.map((problem) => {
                ...
                return (
                  <TiltCard3D
                    key={problem.id}
                    maxTilt={5}
                    glowColor={problem.difficulty === 'advanced' ? '#FF5A43' : problem.difficulty === 'intermediate' ? '#F59E0B' : '#10B981'}
                    soundFeedback={true}
                    onClick={() => onSelectProblem(problem)}
                    className="rounded-2xl bg-[#0e0e15]/95 border border-[#1c1c28] hover:border-[#FF5A43]/50 cursor-pointer shadow-lg"
                  >
                    <div className="p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center space-x-3 self-end sm:self-center">
                        ...
                        >
                          <span>{isSolved ? 'Review Code' : 'Solve Quest'}</span>
